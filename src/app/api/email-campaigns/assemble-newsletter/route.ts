...
import { assembleNewsletterLocalAtomic, EmailCampaignLocalError } from "@/lib/email-campaigns/local";
...
async function assembleNewsletterLocal(topicIds: string[], requestedBy: string) {
  try {
    return NextResponse.json(await assembleNewsletterLocalAtomic({ topicIds, requestedBy }));
  } catch (error) {
