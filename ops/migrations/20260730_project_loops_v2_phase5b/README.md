...
- The app can execute only the claim, heartbeat, runner PID attach, one-time Hermes planner-session bind, completion, terminal-result-bound visual-evidence persistence, and stale-reconcile authority entry points. The work-item transition and visual-evidence guard trigger functions are private.
- Visual evidence rows are inserted only by `persist_visual_qa_evidence(uuid,text)` under the `aipaths_mc_qa_owner` SECURITY DEFINER. The function derives every descriptor and identity field from the terminal immutable `qa_executions.result`; the guard makes those rows immutable and rejects app-authored visual rows while leaving non-visual `loop_evidence` CRUD compatible. Serving re-joins the exact terminal `result`, `result_hash`, `task_id`, and `qa_run_id` before any artifact bytes are read.
- Product completion (`pass`, `changes`, or `failed`) is refused unless `qa_executions.planner_session_id` has been bound exactly once through `bind_visual_qa_planner_session(uuid,text,text)` after the runner verifies the first Hermes planner turn in `state.db`. A pre-planner `infrastructure_failure` may close the execution without inventing a planner session. The existing `qa_session_id` remains the completion identity; `planner_session_id` is an immutable audit binding when planning occurred.
...
6. Run `supabase/migrations/036_visual_qa_runner_v1.sql`; it is mandatory for Visual QA V1 runner use because it installs `pid`, `planner_session_id`, `attach_visual_qa_execution_pid`, `bind_visual_qa_planner_session`, app grants, and the completion-side planner audit check.
7. Install the chosen key through the superuser-only invoker function, using a protected parameter rather than embedding it in a checked-in SQL file:

   ```sql
   SELECT public.install_qa_authority_hmac_key(:'qa_authority_hmac_key');
   ```

8. Set Mission Control's `QA_AUTHORITY_HMAC_KEY` to **the exact same lowercase hex value installed in step 7**. Only a process that signs QA claim envelopes needs this secret; do not broaden it to the scheduler unless that deployment explicitly makes the scheduler a signer. A mismatch makes every signed claim fail closed. Rotation uses the same install function and requires a coordinated drain/cutover because claims signed with the old key stop validating immediately.
9. Run `verify.sql`; it uses `SET TRANSACTION READ ONLY` and checks zero role-membership edges, schema `USAGE`/`CREATE`, ownership, fixed search paths, grants/default ACLs, 036 runner/audit functions and app grants, `planner_session_id` constraints, secret presence without returning key material, integrity triggers, and live row coherence.
10. Set `MISSION_CONTROL_DATABASE_URL=postgres://aipaths_mc_app@127.0.0.1:5432/aipaths_mission_control_local` in both the Mission Control and work-item scheduler service environments. Set the aligned HMAC key only in Mission Control/the designated claim signer. Do not put the key in checked-in plist files, command output, or URLs.
11. Build Mission Control, then reload/restart Mission Control and the scheduler so both processes discard old superuser connections and environment values. Do not enable Phase 5B.1 traffic between the database migration and these two role/env reloads.
12. From each service's effective database URL (or a service health probe that performs the same query), verify `SELECT current_user, rolsuper FROM pg_roles WHERE rolname=current_user` returns `aipaths_mc_app,false`. Then run one scheduler status/health check and one Mission Control health check before re-enabling traffic.
