...
BEGIN
  -- Keep this complete-prefix residue test identical to preflight. Forward must
  -- never accept a helper object that the read-only gate rejects.
  IF EXISTS (
...
BEGIN
  ...
  -- Keep this exact shape/multiplicity test synchronized with the later Loops
  -- index gate. A narrower predicate or included column is not the invariant.
  ...
  IF exact_primary_indexes>1 THEN
    RAISE EXCEPTION 'Loops cutover found % exact primary_execution uniqueness indexes; expected at most one',exact_primary_indexes;
  END IF;
  ...
  IF violations>0 THEN
    RAISE EXCEPTION 'Loops cutover found % source_type CHECKs that are not exactly transformable (each must contain an exact project literal and no loop literal)',violations;
  END IF;

  -- Provenance safety for rollback: every Loop-named constraint/index appearing
  -- later must have been generated by this forward. Reserved fallback names are
  -- included automatically by this broad participating-relation criterion.
  SELECT count(*) INTO violations
  ...
  IF violations>0 THEN
    RAISE EXCEPTION 'Loops cutover found % pre-existing constraint names containing reserved token loop on participating relations',violations;
  END IF;
  SELECT count(*) INTO violations
  FROM pg_indexes i
  ...
  IF violations>0 THEN
    RAISE EXCEPTION 'Loops cutover found % pre-existing index names containing reserved token loop on participating relations',violations;
  END IF;
...
-- checkpoint: recoverable-mutation metadata-created
-- Durable, namespaced recovery metadata is deliberately committed before any
-- mutation when statements are run one-by-one. An empty table records that the
-- source had no source_type CHECK. Rollback consumes it after any partial phase.
DO $cutover_metadata$
BEGIN
  CREATE TABLE public.__mc_loops_cutover_20260726_source_type_checks (
    conname text PRIMARY KEY,
    definition text NOT NULL
  );
  INSERT INTO public.__mc_loops_cutover_20260726_source_type_checks(conname,definition)
  SELECT c.conname,pg_get_constraintdef(c.oid)
  FROM pg_constraint c
  WHERE c.conrelid='public.work_items'::regclass AND c.contype='c'
    AND c.conkey @> ARRAY[(SELECT attnum FROM pg_attribute WHERE attrelid=c.conrelid AND attname='source_type')]::smallint[];
END $cutover_metadata$;

-- checkpoint: recoverable-mutation source-checks-widened
DO $$
DECLARE
  obj record;
  inner_definition text;
BEGIN
  FOR obj IN SELECT conname,definition FROM public.__mc_loops_cutover_20260726_source_type_checks LOOP
...
-- checkpoint: recoverable-mutation rewrite-helper-created
CREATE FUNCTION public.__mc_loops_cutover_20260726_rewrite_project_values(value jsonb)
  ...
  CASE jsonb_typeof(value)
    WHEN 'string' THEN
      ...
      RETURN to_jsonb(CASE scalar
        ...
        ELSE scalar END);
    WHEN 'array' THEN
      SELECT coalesce(jsonb_agg(public.__mc_loops_cutover_20260726_rewrite_project_values(item)), '[]'::jsonb)
        INTO result FROM jsonb_array_elements(value) AS items(item);
      RETURN result;
    WHEN 'object' THEN
      SELECT coalesce(jsonb_object_agg(key, public.__mc_loops_cutover_20260726_rewrite_project_values(item)), '{}'::jsonb)
...
-- checkpoint: recoverable-mutation work-items-payload-rewritten
-- Rewrite controlled JSON keys and exact controlled scalar values only.
UPDATE public.work_items SET payload = public.__mc_loops_cutover_20260726_rewrite_project_values(
...
-- checkpoint: recoverable-mutation project-events-payload-rewritten
UPDATE public.project_events SET payload = public.__mc_loops_cutover_20260726_rewrite_project_values(
  (payload - 'source_project_id' - 'source_project_title' - 'materialized_from_project' - 'project_status_at_materialization' - 'superseded_for_project_id')
  || CASE WHEN payload ? 'source_project_id' THEN jsonb_build_object('source_loop_id',payload->'source_project_id') ELSE '{}'::jsonb END
  || CASE WHEN payload ? 'source_project_title' THEN jsonb_build_object('source_loop_title',payload->'source_project_title') ELSE '{}'::jsonb END
  || CASE WHEN payload ? 'materialized_from_project' THEN jsonb_build_object('materialized_from_loop',payload->'materialized_from_project') ELSE '{}'::jsonb END
...
-- checkpoint: recoverable-mutation projects-metadata-rewritten
UPDATE public.projects SET metadata=public.__mc_loops_cutover_20260726_rewrite_project_values(metadata)
...
-- checkpoint: recoverable-mutation terminal-orphans-marked
...
-- checkpoint: recoverable-mutation work-item-values-rewritten
...
-- checkpoint: recoverable-mutation source-checks-finalized
DO $$
DECLARE
  obj record;
  replacement text;
BEGIN
  ...
  END LOOP;
  IF NOT EXISTS (SELECT 1 FROM public.__mc_loops_cutover_20260726_source_type_checks) THEN
...
-- checkpoint: recoverable-mutation project-event-values-rewritten
...
-- checkpoint: recoverable-mutation namespace-renamed
-- Every namespace rename shares one statement and therefore one transaction
-- even under SQL Editor autocommit.
DO $namespace_rename$ BEGIN
  ...
  END IF;
END $namespace_rename$;

-- checkpoint: recoverable-mutation object-names-renamed
DO $$
DECLARE
  obj record;
  next_name text;
...
-- checkpoint: recoverable-mutation primary-execution-index-ensured
DO $$
...
-- checkpoint: recoverable-mutation helpers-cleaned
-- Successful forward leaves no helper objects. The DROP pair is itself atomic.
DO $cutover_cleanup$ BEGIN
  DROP FUNCTION public.__mc_loops_cutover_20260726_rewrite_project_values(jsonb);
  DROP TABLE public.__mc_loops_cutover_20260726_source_type_checks;
END $cutover_cleanup$;
RESET lock_timeout;
