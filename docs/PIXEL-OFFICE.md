...
```
Supabase (agent_tasks, memories[type=journal])
  ↓ SSR fetch (page.tsx)
...
1. Primary: Supabase Realtime (`postgres_changes` on `agent_tasks` + `memories` filtered to `type=journal`)
2. Fallback: If channel doesn't reach "SUBSCRIBED" in 5s, start polling every 10s from `agent_tasks` and `memories` where `type='journal'`
3. Prerequisite: Enable Realtime on both tables in Supabase dashboard

## Memory Source

Pixel Office reads memory bubbles from `memories` rows where `type='journal'`, ordered by `date` descending and then `created_at` descending. The older `agent_memory` table is legacy.
